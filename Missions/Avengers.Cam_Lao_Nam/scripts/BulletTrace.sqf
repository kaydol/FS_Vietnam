...
if ( isNil{ missionNameSpace getVariable "hyp_var_tracer_currentIndex" } ) then {
	missionNameSpace setVariable ["hyp_var_tracer_currentIndex", 0, TRUE];
};	
if ( isNil{ missionNameSpace getVariable "hyp_var_tracer_activeIndexes" } ) then {	
    missionNameSpace setVariable ["hyp_var_tracer_activeIndexes", [], TRUE];
};	
...
{
	{
        private _unit = _x;
        {
           private  _positions = missionNameSpace getVariable [format["hyp_var_tracer_projectile_%1", _x], []];
           private  _color     = _unit getVariable ["hyp_var_tracer_color", [1,0,0,1]];
           private  _muzzleVelocity = _positions select 0 select 1;
            ...
            for "_i" from 0 to (count _positions) - 2 do 
			{
				...
				{
                    private _velocity = (_positions select _i) select 1;
		...
		forEach ( missionNameSpace getVariable "hyp_var_tracer_activeIndexes" );
    }
	forEach (AllPlayers select {_x getVariable ["hyp_var_tracer_initialized", FALSE]});
}];
...
hyp_fnc_traceFire = {
    ...
    _unit setVariable ["hyp_var_tracer_color", _color, TRUE];
    _unit setVariable ["hyp_var_tracer_lifetime", _lifetime, TRUE];
    _unit setVariable ["hyp_var_tracer_interval", _interval, TRUE];
    _unit setVariable ["hyp_var_tracer_trackVel", _trackVel, TRUE];
    _unit setVariable ["hyp_var_tracer_maxDistance", _maxDistance, TRUE];
    _unit setVariable ["hyp_var_tracer_maxDuration", _maxDuration, TRUE];
    _unit setVariable ["hyp_var_tracer_initialized", TRUE, TRUE];

	_unit remoteExec ["hyp_fnc_serverEventHandler", 2];
};

hyp_fnc_serverEventHandler = {
	params ["_unit"];
	...
	_unit addEventHandler ["Respawn", 
	{
		...
		_corpse removeAllEventHandlers "Fired";
		_corpse removeAllEventHandlers "Respawn";
		[_unit, nil, 3, 2, 200, 3, true] remoteExec ["hyp_fnc_traceFire", 0, TRUE]; // this is the exact same line used in PerksMenu.sqf
    }];
};
...
hyp_fnc_traceFireEvent = {
    ...
    _projIndex     = missionNameSpace getVariable "hyp_var_tracer_currentIndex";  //Get the index to assign to the bullet
    missionNameSpace setVariable ["hyp_var_tracer_currentIndex", _projIndex + 1, TRUE]; //Increment index for next bullet
	
    //Initialize final array into which all positions for the current projectile will be stored...
    missionNameSpace setVariable [format["hyp_var_tracer_projectile_%1", _projIndex], _positions, TRUE];
    //...Then update the activeIndexes to indicate that the projectile is active
    _activeIndexes = missionNameSpace getVariable "hyp_var_tracer_activeIndexes";
    _activeIndexes set [count _activeIndexes, _projIndex];
    missionNameSpace setVariable ["hyp_var_tracer_activeIndexes", _activeIndexes, TRUE];
    ...
    if (_lifetime != -1) then {
        ...
        missionNameSpace setVariable [format["hyp_var_tracer_projectile_%1", _projIndex], nil, TRUE]; //Delete the projectile's data
	};
};
