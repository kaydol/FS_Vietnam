...
Synced objects:
	...
	Vehicles:					If vehicles are synchronized, attempt to spawn player 
								as passenger (vehicle must be alive), otherwise player 
								will spawn in general vicinity of the vehicle.							
...
if ( _deleteOldBody || _spawnAsPassenger || _respawnFromDarkness > 0 || count _respawnAnimations > 0 || _vfxLength > 0 || _godModeLength > 0 || _landSearchRadius > 0 ) then {
		...
		_respawnEH = player addEventHandler ["Respawn", {
			_this spawn 
			{
				...
				if (_spawnAsPassenger && alive _vehicle && !(_vehicle isKindOf "Module_F")) then 
				{
					...
					private _posInWater = (position _unit) call _isPosInWater;
